...
func remove_item(item_to_remove):
	...
	weight_sum = items.reduce(func (accum, item): return accum + item["weight"], 0)
...
func pick_item(exclude: Array = []):
	...
	if exclude.size() > 0:
		adjusted_items = items\
			.filter(func (item): return not item["item"] in exclude)
		adjusted_weight_sum = adjusted_items\
			.reduce(func (accum, item): return accum + item["weight"], 0)
